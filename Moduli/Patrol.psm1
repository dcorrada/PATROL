function Patrol {
    ...
    Import-Module -Name '[percorso_di_installazione]\PATROL\Moduli\Forms.psm1'
    ...
    if ((new-object directoryservices.directoryentry "",$usr,$MaskedTextBox.Text).psbase.name -ne $null) {
        ...
        Import-Module -Name '[percorso_di_installazione]\PATROL\Moduli\FileCryptography.psm1'

        Copy-Item -Path "[percorso_di_installazione]\PATROL\PatrolDB.csv.AES" -Destination "C:\Users\$env:USERNAME\Desktop\PatrolDB.csv.AES"
        $stringa = Get-Content '[percorso_di_installazione]\PATROL\crypto.key'
    ...
    Start-Sleep 3
    New-PSDrive -Name Z -PSProvider FileSystem -Root '[percorso_di_installazione]\PATROL' -Credential $credit > $null
